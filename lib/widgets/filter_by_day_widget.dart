...
class FilterComponent extends StatefulWidget {
  ...
  const FilterComponent({Key? key, this.onSelectionChange}) : super(key: key);
...
class _FilterComponentState extends State<FilterComponent> {
  ...
  Future<void> _selectDateRange() async {
    ...
    final picked = await showDateRangePicker(
      context: context,
      initialEntryMode: DatePickerEntryMode.calendarOnly, // Shows only calendar
      firstDate: DateTime(1900),
      lastDate: DateTime.now(),
      cancelText: translate(TextHelper.cancel),
      barrierColor: Colors.black54, // Dim background
      builder: (context, child) {
        return Dialog( // Wrap in Dialog for full styling control
            ...
            child: Theme(
              data: theme.copyWith(
                ...
                textTheme: TextTheme(
                  ...
                  bodySmall: TextStyle(color: Colors.white), // Labels text color
                ),
                textButtonTheme: TextButtonThemeData(
                  style: TextButton.styleFrom(foregroundColor: Colors.pink), // Button text color
                ),
                dialogBackgroundColor: AppColors.darkBg2, // Ensures full background matches theme
              ),
              child: ClipRRect(
                borderRadius: BorderRadius.circular(12),
                child: child!,
    ...
    if (picked != null) {
      setState(() {
        selectedKey = "custom";
        selectedCustomRange = DateTimeRange(start: picked.start, end: picked.end);
      });
  ...
  DateTimeRange _getDateRange(String key) {
    ...
    switch (key) {
      ...
      case "last7":
        return DateTimeRange(start: today.subtract(Duration(days: 6)), end: today);
      case "last30":
        return DateTimeRange(start: today.subtract(Duration(days: 30)), end: today);
      case "last90":
        return DateTimeRange(start: today.subtract(Duration(days: 90)), end: today);
      case "last365":
        return DateTimeRange(start: today.subtract(Duration(days: 365)), end: today);
      case "lastMonth":
          ...
          children: ranges.map((item) {
            ...
            return GestureDetector(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  color: isSelected ? Color(0xFFF90182) : Color(0xFF141E2D),
                  borderRadius: BorderRadius.circular(50),
                  border: Border.all(color: Color(0xFFA3AED0).withOpacity(0.4)),
                ),
                child: Row(
                  children: [
                    ...
                    if (item["key"] == "custom") ...[
                      ...
                      if (selectedCustomRange != null)
                        Text(
                          "${formatter.format(selectedCustomRange!.start)} - ${formatter.format(selectedCustomRange!.end)}",
                          style: TextStyle(
                            color: Colors.white,
                      ...
                      Text(
                        item["label"]!,
                        style: TextStyle(
                          color: isSelected ? Colors.white : Color(0xFFA3AED0),
                          fontWeight: FontWeight.w600,
                        ),
                      ),
                  ],
